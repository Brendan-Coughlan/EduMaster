        ...
        while (!exitProgram) {
            ...
            switch (input) {
                case "CHANGE PASSWORD":
                    ...
                    break;
                case "CREATE":
                    if(getPersonByID(id).getRole() == Person.Role.Student || getPersonByID(id).getRole() == Person.Role.Teacher) {
    ...
    private void createProfile(Scanner scnr) {
        ...
        while (true) {
            ...
            try {
                ...
                switch(role) {
                    case "Student":
                        newPerson = new Student(String.format("%09d", id+1), Person.Role.valueOf(role), firstName.charAt(0) + lastName,
                        firstName, lastName);
                    students.add((Student) newPerson);
                    break;
                case "Teacher":
                    newPerson = new Teacher(String.format("%09d", id+1), Person.Role.valueOf(role), firstName.charAt(0) + lastName,
                    firstName, lastName);
                    teachers.add((Teacher) newPerson);
                    break;
                case "Admin":
                    newPerson = new Admin(String.format("%09d", id+1), Person.Role.valueOf(role), firstName.charAt(0) + lastName,
                    firstName, lastName);
                    admins.add((Admin) newPerson);
                    break;
                }
